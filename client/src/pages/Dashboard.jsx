...
import { AlertTriangle, CloudRain, Wind, Shield, MessageSquare, Phone, MapPin, Camera } from '../icons/Icons';
  ...
  return (
    <div className="relative flex flex-col items-center justify-center py-6">
      <div className="w-48 h-24 overflow-hidden relative">
        ...
        <div 
          className={`w-48 h-48 rounded-full border-[16px] absolute top-0 left-0 box-border transition-all duration-1000 ease-out border-t-transparent border-r-transparent border-l-transparent ${getRiskColor().split(' ')[1]}`} 
          style={{ transform: `rotate(${parseInt(getRiskPercentage()) * 1.8 - 45}deg)` }}
        >
        </div>
      </div>
...
// Mock data for water level chart
...
export const Dashboard = () => {
  ...
  const [alerts, setAlerts] = useState([
    { id: '1', type: 'warning', severity: 'high', title: 'River Level Rising', message: 'Water levels at East Bank have exceeded 2.0m. Prepare for potential evacuation.', timestamp: '10 min ago' },
  ]);
  ...
  useEffect(() => {
    const initWeather = async () => {
      ...
      try {
        ...
        if (data.stats.rainfall > 50 || data.stats.windSpeed > 100) setRiskLevel('CRITICAL');
        else if (data.stats.rainfall > 20 || data.stats.windSpeed > 60) setRiskLevel('HIGH');
        else if (data.stats.rainfall > 5 || data.stats.windSpeed > 30) setRiskLevel('MODERATE');
        ...
        setWeather(prev => ({ ...prev, condition: 'Unavailable', locationName: 'Offline Mode' }));
      } finally {
    ...
    <div className="max-w-7xl mx-auto pb-20">
      ...
      <button 
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 bg-blue-600 hover:bg-blue-500 text-white rounded-full shadow-lg flex items-center justify-center transition-transform hover:scale-105 z-30 ${
          riskLevel === 'HIGH' || riskLevel === 'CRITICAL' ? 'animate-pulse shadow-red-500/50 ring-4 ring-red-500/30 bg-red-600 hover:bg-red-500' : 'shadow-blue-500/40'
        }`}
      >
